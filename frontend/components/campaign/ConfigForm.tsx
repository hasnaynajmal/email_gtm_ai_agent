...
export function ConfigForm({
  ...
  const categoryOptions = Object.keys(options.company_categories).map(key => ({
    value: key,
    label: key,
  }));
  ...
  const personalizationOptions = options.personalization_levels.map(level => ({
    value: level,
    label: level,
  }));
    ...
    <div className="space-y-6">
      ...
      <div>
        <Select
          ...
          onChange={(e) => onChange({ ...config, company_category: e.target.value })}
          options={categoryOptions}
          helperText="Type of companies you want to target"
        />
        {selectedCategory && (
          <div className="mt-2 p-3 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-900 font-medium mb-1">About this category:</p>
            <p className="text-sm text-blue-800">{selectedCategory.description}</p>
            <p className="text-xs text-blue-700 mt-2">
              <span className="font-medium">Typical roles:</span> {selectedCategory.typical_roles.join(', ')}
            </p>
          </div>
        )}
      </div>
      ...
      <Select
        ...
        onChange={(e) => onChange({ ...config, service_type: e.target.value as OutreachConfig['service_type'] })}
      ...
      <Select
        ...
        onChange={(e) => onChange({ ...config, company_size_preference: e.target.value as OutreachConfig['company_size_preference'] })}
      ...
      <div>
        <Select
          ...
          onChange={(e) => onChange({ ...config, personalization_level: e.target.value as OutreachConfig['personalization_level'] })}
          options={personalizationOptions}
          helperText="Depth of research and personalization"
        />
        <div className="mt-2 text-sm text-gray-600">
          {config.personalization_level === 'Basic' && '• Quick research with basic company info'}
          {config.personalization_level === 'Medium' && '• Moderate research with company details and recent news'}
          {config.personalization_level === 'Deep' && '• Comprehensive research with detailed insights and competitive analysis'}
        </div>
      </div>
